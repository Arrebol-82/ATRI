...
const { data: products, pending, error } = await useFetch<MerchandiseProduct[]>('/api/merchandise', {
  default: () => []
})
...
function handleStatusChange(product: MerchandiseProduct, action: 'list' | 'unlist') {
...
<template>
  <div
    ...
    :class="isDark ? 'products-dark bg-[#07080d] text-gray-100' : 'bg-transparent'"
  >
    <AdminHeader title="商品管理" :is-dark="isDark" />
        ...
        >
          ...
          >
            <button
              ...
              :class="{ 'bg-[#5b4eff]/10 text-[#5b4eff]': selectedCategory === category }"
              @click="selectCategory(category)"
            >
              <span v-if="selectedCategory === category" class="mr-2">✓</span>
              <span :class="{ 'ml-5': selectedCategory !== category }">{{ category }}</span>
            </button>
          </div>
        </Transition>
      </div>
    ...
    <div v-if="pending" class="rounded-[24px] bg-white py-20 text-center text-sm font-bold text-[#5b4eff] shadow-sm ring-1 ring-gray-100">
      正在连接数据库...
    </div>

    <div v-else-if="error" class="rounded-[24px] bg-white py-20 text-center text-sm font-bold text-red-500 shadow-sm ring-1 ring-gray-100">
      商品数据加载失败
    </div>

    <div v-else-if="!filteredProducts.length" class="rounded-[24px] bg-white py-20 text-center text-sm font-bold text-gray-500 shadow-sm ring-1 ring-gray-100">
      没有找到匹配的商品
    </div>
    ...
    <div v-else class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      ...
      >
        <div v-if="!product.name.includes('Acrylic Panel')" class="relative mb-4 flex aspect-[4/3] w-full items-center justify-center overflow-hidden rounded-[16px] bg-[#f4f5f7] p-3">
          <img
        ...
        <div class="flex items-center gap-2 text-xs font-black tracking-[0.06em] text-[#5b4eff]">
          <span class="h-2.5 w-2.5 rotate-45 rounded-[2px] bg-[#5b4eff]" />
          <span>{{ product.category }}</span>
        </div>

        <h3 class="mt-3 line-clamp-2 min-h-[48px] text-base font-black leading-6 text-gray-800" :title="product.name">
          {{ product.name }}
        </h3>

        <div class="mt-3 flex flex-col gap-1 text-xs font-medium text-gray-500">
          <span>价格: {{ product.priceDisplay }}</span>
          <span>创建时间: {{ formatCreatedAt(product.createdAt) }}</span>
        </div>
